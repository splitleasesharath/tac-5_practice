...
Generate ONE interesting natural language query that explores the data in this database.

Requirements:
- The query should be relevant to the actual tables and columns in the schema
- Make the query interesting by using different SQL capabilities: filtering, aggregation, grouping, sorting, or joins
- Use actual table names and column names from the schema
- Keep it concise - maximum TWO sentences
- Return ONLY the natural language query text, no SQL code
- Do not include any explanations or preamble
- Make it diverse - vary between simple queries, aggregations, filters, and analytical questions
...
Natural Language Query:"""
        ...
        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that generates interesting natural language database queries based on schema information."},
                {"role": "user", "content": prompt}
            ],
...
def generate_random_query(schema_info: Dict[str, Any], provider: str = "openai") -> str:
    """
    ...
    Priority: 1) OpenAI if key exists, 2) Anthropic if key exists, 3) provider parameter
    ...
    Returns:
        str: Natural language query string (max 2 sentences)

    Raises:
        Exception: If no tables in schema or API call fails
    """
    ...
    # Fall back to provider preference if both keys available or neither available
    if provider == "openai":
        return generate_random_query_with_openai(schema_info)
    else:
        return generate_random_query_with_anthropic(schema_info)