...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    # Set up environment - use full environment inheritance (Option 2)
    # This ensures Windows system variables (SYSTEMROOT, etc.) are available for Node.js crypto
    env = os.environ.copy()
    # Still add GH_TOKEN if needed
...
def execute_template(request: AgentTemplateRequest) -> AgentPromptResponse:
    ...
    # For large arguments, save to file instead of passing directly
    args_to_use = []
    for i, arg in enumerate(request.args):
        if len(arg) > 1000:  # If arg is large (>1000 chars)
            arg_file = os.path.join(output_dir, f"arg_{i}.txt")
            with open(arg_file, "w") as f:
                f.write(arg)
            args_to_use.append(arg_file)
            print(f"Saved large argument to: {arg_file}")
        else:
            args_to_use.append(arg)

    # Construct prompt from slash command and args
    prompt = f"{request.slash_command} {' '.join(args_to_use)}"
